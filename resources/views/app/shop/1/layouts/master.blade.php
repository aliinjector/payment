                                    ...
                                    <ul class="dropdown-menu multi-level font-16" role="menu" aria-labelledby="dropdownMenu" style="right:.2em!important">
                                        ...
                                        <li class="dropdown-submenu">
                                            ...
                                            <ul class="dropdown-menu font-16">
                                                ...
                                                @if (!$subSubCategory->children()->exists())
                                                <a tabindex="-1" href="{{ route('category', ['shop'=>$shop->english_name, 'categroyId'=>$subSubCategory->id, 'name' => $subSubCategory->name]) }}">
                                                    <li class="dropdown-item @if( Request::is('*/category/'.$subSubCategory->id.'/*')) bg-omid-orange @endif @if(Route::currentRouteName() == 'category' and $CategoryCTLR->getAllSubCategories($subSubCategory->id)->contains('id', explode('/',url()->current())[5])) border-bottom border-omid-orange @endif">{{ $subSubCategory->name }}</li>
                                                </a>
                                                @else
                                                <li class="dropdown-submenu">
                                                    <a href="{{ route('category', ['shop'=>$shop->english_name, 'categroyId'=>$subSubCategory->id, 'name' => $subSubCategory->name]) }}" class="dropdown-item pointer-crouser @if( Request::is('*/category/'.$subSubCategory->id.'/*')) bg-omid-orange @endif @if(Route::currentRouteName() == 'category' and $CategoryCTLR->getAllSubCategories($subSubCategory->id)->contains('id', explode('/',url()->current())[5])) border-bottom border-omid-orange @endif"
                                                    ...
                                                    <ul class="dropdown-menu font-16">
                                                        ...
                                                        <a href="{{ route('category', ['shop'=>$shop->english_name, 'categroyId'=>$subSubSubCategory->id, 'name' => $subSubSubCategory->name]) }}" style="color: #465f73!important;">
                                                            <li class="dropdown-item @if( Request::is('*/category/'.$subSubSubCategory->id.'/*')) bg-omid-orange @endif @if(Route::currentRouteName() == 'category' and $CategoryCTLR->getAllSubCategories($subSubSubCategory->id)->contains('id', explode('/',url()->current())[5])) border-bottom border-omid-orange @endif">{{ $subSubSubCategory->name }}</li>
                                                        </a>
                                                        @endforeach
                                                    </ul>
                                                    @endif
