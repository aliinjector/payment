...
@section('content')
    ...
    @if(isset($products[0]))
        <h2 class="line-throw line-height-none">
            <span> محصولات دسته بندی {{ $products[0]->productCategory()->get()->first()->name }}</span></h2>
    @else
        ...
        <div class="col-lg-3">
            <div class="card e-co-product" style="max-width: 18rem;">
                ...
                <div class="card-body d-flex justify-content-center text-primary">
                    <form
                        action="{{ route('shop.show.category', ['shop' => $shop->english_name,'categroyId' => $category]) }}"
                        id="submit" method="get">
                        <div class="btn-group btn-group-toggle mb-4 flex-wrap" data-toggle="buttons">
                            <label id="available-filter-1" for="available-filter-1"
                                   class="border-top-down-radius-0 btn btn-outline-secondary @if(request()->type == '') active @endif border-left-0 iranyekan crouser"
                                   style="cursor:pointer">
                                <input type="radio" name="type" value="all" id="available-filter-1"
                                       @if(request()->type == '' or request()->type == 'all') checked="" @endif> همه
                            </label>
                            <label id="available-filter-2" for="available-filter-2"
                                   class="border-top-down-radius-0 btn btn-outline-secondary border-right-0  @if(request()->type == 'product') active @endif border-left-0 iranyekan"
                                   style="cursor:pointer">
                                <input type="radio" name="type" value="product" id="available-filter-2"
                                       @if(request()->type == 'product') checked="" @endif> فیزیکی
                            </label>
                            <label id="available-filter-3" for="available-filter-3"
                                   class="border-top-down-radius-0 btn btn-outline-secondary border-right-0  @if(request()->type == 'file') active @endif border-left-0 iranyekan"
                                   style="cursor:pointer">
                                <input type="radio" name="type" value="file" id="available-filter-3"
                                       @if(request()->type == 'file') checked="" @endif> فایل
                            </label>
                            <label id="available-filter-4" for="available-filter-4"
                                   class="border-top-down-radius-0 btn btn-outline-secondary  border-right-0  @if(request()->type == 'service') active @endif iranyekan"
                                   style="cursor:pointer">
                                <input type="radio" name="type" value="service" id="available-filter-4"
                                       @if(request()->type == 'service') checked="" @endif> خدماتی
                            </label>
                        </div>
                        <h5 class="text-dark pr-1 border-btm font-weight-500 m-2">فیلتر بر اساس قیمت کالا</h5>
                        <input type="text" id="available-price-1" class="w-100 p-2 iranyekan font-14"
                               style="border:0; color:#F68712 !important; font-weight:bold;">
                        <input type="hidden" id="available-price-min" name="minprice"
                               value="@if(request()->minprice == null) 1000 @else {{ request()->minprice }} @endif">
                        <input type="hidden" id="available-price-max" name="maxprice"
                               value="@if(request()->maxprice == null) 1000000 @else {{ request()->maxprice }} @endif">
                        </h5>
                        <div id="mySlider"></div>
                </div>
            </div>
                ...
                <div class="border-0">
                    <div class="list-group list-group-root well border-0">
                        @foreach($categories as $category)
                            <a href="#item-{{$category->id}}"
                               class="border-0 iranyekan p-0 py-1 @if( Request::is('*/category/'.$category->id)) font-weight-bolder text-dark @endif"
                               data-toggle="collapse">
                                <i class="fa fa-angle-left light-dark-text-color font-12 mr-2"></i>{{ $category->name }}
                            </a>
                            <div
                                class="list-group collapse border-0  @if( Request::is('*/category/'.$category->id)) show @endif"
                                id="item-{{$category->id}}">
                                {{-- @if($category->children()->exists()) --}}
                                @foreach ($category->children()->get() as $subCategory)
                                    <a href="#item-{{$category->id}}-{{$subCategory->id}}"
                                       class="border-0 iranyekan  dark-text-color p-2" data-toggle="collapse">
                                        <i class="fa fa-angle-down light-dark-text-color font-12 mr-2"></i> {{ $subCategory->name}} </br>
                                    </a>
                                    <div class="list-group collapse border-0"
                                        ...
                                        @foreach ($subCategory->children()->get() as $subSubCategory)
                                            <a href="#item-{{$category->id}}-{{$subCategory->id}}-{{$subSubCategory->id}}"
                                               class="border-0 iranyekan dark-text-color p-2 mr-5"
                                               data-toggle="collapse">{{ $subSubCategory->name}}</a>
                                            <div class="list-group collapse border-0"
                                                 id="item-{{$category->id}}-{{$subCategory->id}}-{{$subSubCategory->id}}">
                                                {{-- @if($subCategory->children()->exists() and $category->children()->get()->first()->children()->exists()) --}}
                                                @foreach ($subSubCategory->children()->get() as $subSubSubCategory)
                                                    <a href="#item-{{$category->id}}-{{$subCategory->id}}-{{$subSubCategory->id}}-{{$subSubSubCategory->id}}"
                                                       class="border-0 iranyekan dark-text-color p-2 mr-5 pr-4"
                                                       data-toggle="collapse">{{ $subSubSubCategory->name}}
                                                    </a>
                                                    <div class="list-group collapse border-0"
                                                         id="item-{{$category->id}}-{{$subCategory->id}}-{{$subSubCategory->id}}-{{$subSubSubCategory->id}}">
                                                        {{-- @if($subCategory->children()->exists() and $category->children()->get()->first()->children()->exists()) --}}
                                                        @foreach ($subSubSubCategory->children()->get() as $subSubSubSubCategory)
                                                            <a href="#"
                                                               class="border-0 iranyekan dark-text-color p-2 mr-5  pr-5">{{ $subSubSubSubCategory->name}}
                                                            </a>
                                                        @endforeach
                                                    </div>
                                                @endforeach
            ...
            <div class="row d-flex justify-content-center mr-2">
                ...
                <div class="btn-group btn-group-toggle mb-4 flex-wrap d-inline" data-toggle="buttons">
                    <label id="available-order-1" for="available-order-1"
                           ...
                           style="cursor:pointer">
                        <input type="radio" class="available-order-1" id="available-order-1" name="sortBy[field]"
                               value="created_at"
                               @if(request()->sortBy['field'] == '' or request()->sortBy['field'] == 'created_at') checked="" @endif>
                        جدید ترین ها
                    ...
                    <label id="available-order-2" for="available-order-2"
                           ...
                           style="cursor:pointer">
                        <input type="radio" class="available-order-2" id="available-order-2" name="sortBy[field]"
                               value="price"
                               @if(request()->sortBy['field'] == 'price' and request()->sortBy['orderBy'] == 'asc') checked="" @endif>
                        ارزان ترین ها
                    ...
                    <label id="available-order-3" for="available-order-3"
                           ...
                           style="cursor:pointer">
                        <input type="radio" class="available-order-3" id="available-order-3" name="sortBy[field]"
                               value="price"
                               @if(request()->sortBy['field'] == 'price' and request()->sortBy['orderBy'] == 'desc') checked="" @endif>
                        گران ترین ها
                        <input type="radio" class="available-order-3" name="sortBy[orderBy]" value="desc">
                    </label>
                    <label id="available-order-4" for="available-order-4"
                           class="btn btn-outline-orange @if(request()->sortBy['field'] == 'buyCount')  active @endif border-0 rounded px-2 mx-2 iranyekan color-blue font-weight-bold"
                           style="cursor:pointer">
                        <input type="radio" class="available-order-4" id="available-order-4" name="sortBy[field]"
                               value="buyCount" @if(request()->sortBy['field'] == 'buyCount') checked="" @endif> پرفروش
                        ترین ها
                ...
                @foreach ($productsPaginate as $product)
                    <div class="col-lg-3 row">
                        <div class="card e-co-product min-height-60 col-lg-12">
                            <a href="{{ route('shop.show.product', ['shop'=>$shop->english_name, 'id'=>$product->id]) }}"><img
                                    src="{{ $product->image['250,250'] }}" alt="" class="img-fluid"></a>
                            <div class="card-body product-info"><a
                                    href="{{ route('shop.show.product', ['shop'=>$shop->english_name, 'id'=>$product->id]) }}"
                                    class="product-title">{{ $product->title }}</a>
                                <div class="d-flex justify-content-between my-2 byekan">
                                    @if($product->off_price != null)
                                        <p class="product-price byekan">{{  number_format($product->off_price) }} تومان
                                            <span class="ml-2 byekan"></span><span class="ml-2"><del
                                                    class="byekan font-16">{{  number_format($product->price) }} تومان</del></span>
                                        </p>
                                    @else
                                        <p class="product-price byekan">{{  number_format($product->price) }} تومان
                                            <span class="ml-2 byekan"></span>
                                    @endif
                                </div>
                            ...
                            @if(\Auth::user())
                                    <form
                                        action="{{ route('cart.add', ['shop'=>$shop->english_name, 'userID'=> \Auth::user()->id]) }}"
                                        method="post">
                                        @csrf
                                        <input type="hidden" name="product_id" value="{{$product->id}}">
                                        <button type="submit"
                                                class="btn btn-cart btn-sm waves-effect waves-light iranyekan"><i
                                                class="mdi mdi-cart mr-1"></i> @if($product->type == 'file') دریافت
                                            فایل  @else اضافه به سبد خرید @endif</button>
                                    </form>
                                @endif
                            </div>
                            <!--end card-body-->
